...
console_app: Typer = Typer(
    help="Консольная утилита для сортировки файлов по типам"
         "В папке"
)
        ...
        with self._console_manager.progress as progress:
            ...
            for i, file in enumerate(self._path_model.files):
                ...
                progress.update(
                    ...
                    description=
                    f"[cyan] Сортировка... "
                    f"({i} / {self._path_model.files_count})"
                )
...
@console_app.command()
def sorter(source: Path = Argument(..., help="Путь к папке"),
           silent: bool = Option(
               False, "--silent",
               help="Не выводить логи"
           )
