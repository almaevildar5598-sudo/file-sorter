...
class Sorter:
    """
    Класс для сортировки файлов в каталоге
    ...
    def sort(self) -> None:
        ...
        Examples:
            ...
            >>> sort_inst: Sorter = Sorter(Path("C:/1/2/3"), False)
            >>> # Сортировка без вывода результата
            >>> sort_inst1: Sorter = Sorter(Path("C:/1/2/3"), True)
        """
        with self._console_manager.progress as progress:
            ...
            for i, file in enumerate(self._path_model.files):
                ...
                progress.update(task, description=f"[cyan] Сортировка... " f"({i} / {self._path_model.files_count})")
